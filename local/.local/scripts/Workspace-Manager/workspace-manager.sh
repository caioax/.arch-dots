...
readonly CONFIG_FILE="$HOME/.config/hypr/conf.d/workspaces.conf"
...
# Verifica se o source já existe no hyprland.conf
check_source_exists() {
    local hyprland_conf="$HOME/.config/hypr/hyprland.conf"
    local config_basename
    config_basename=$(basename "$CONFIG_FILE")

    # Verifica se o arquivo hyprland.conf existe
    if [[ ! -f "$hyprland_conf" ]]; then
        return 1
    fi

    # Padrões para detectar se já existe (cobre várias variações):
    # - source = ~/.config/hypr/conf.d/workspaces.conf
    # - source=~/.config/hypr/conf.d/workspaces.conf
    # - source = $HOME/.config/hypr/conf.d/workspaces.conf
    # - source = /home/user/.config/hypr/conf.d/workspaces.conf
    # - Linhas comentadas são ignoradas (não começam com #)

    # Remove comentários e verifica padrões
    grep -v '^[[:space:]]*#' "$hyprland_conf" 2>/dev/null |
        grep -qE "^[[:space:]]*source[[:space:]]*=[[:space:]]*.*(conf\.d/workspaces\.conf|${config_basename})[[:space:]]*$"
}

# Adiciona o source ao hyprland.conf
add_source_to_config() {
    ...
    # Adiciona source ao final do arquivo
    {
        echo ""
...
HEADER
    ...
    # Verifica se já está sendo usado no hyprland.conf
    if check_source_exists; then
        ...
        # Só pergunta se estiver em terminal interativo
        if [[ -t 0 ]]; then
            ...
            log_info "⚠ Executando em modo não-interativo, adicione manualmente."
        fi
    fi
...
status() {
    ...
    # Pega dados dos monitores uma única vez para evitar múltiplas chamadas
    local monitors_json
    monitors_json=$(hyprctl monitors -j)
    ...
    for idx in "${!monitor_array[@]}"; do
        ...
        local current_ws
        current_ws=$(echo "$monitors_json" | jq -r --arg mon "$monitor" '.[] | select(.name == $mon) | .activeWorkspace.id')
    ...
    echo ""
    echo "Source no hyprland.conf:"
    if check_source_exists; then
        echo "  ✓ Configurado"
    else
        echo "  ✗ Não configurado (rode --init)"
    fi
}
